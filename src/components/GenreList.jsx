        ...
        <div>
...

            {
                genreList.map((item, index) => (
                    <div key={item.id}
                        onClick={ () => { setActiveLink(index); genreID(item.id); }} 
                    ...
                    >
                        <img src={item.image_background} 
                            ...
                            alt={item.name}
                        />
                        <h3 className={`text-[13px] group-hover:font-bold transition-all ease-out duration-200 ${activeLink === index ? 'font-bold text-[20px]' : 'font-normal text-[13px]'}`}>
                            {item.name}
                        </h3>    
                    </div>
                ))
